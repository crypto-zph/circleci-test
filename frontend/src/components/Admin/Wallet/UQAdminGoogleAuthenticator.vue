<template>
  <div v-if="is_2fa_enabled != null" :class="$route.name == 'admin_google_auth' ? 'google_authClass' : ''">
    <div v-if="is_2fa_enabled == false">
      <h6 style="font-size: 18px" class="text-bold">
        Make your admin account more secure.
      </h6>
      ...
      <div class="row">
        ...
        <img
          ...
          alt="2FA Secret Key" style="border-radius: 20px"
        />
      </div>
      ...
      <!-- 2FA Code Input -->
      <div class="row">
        <div :class="$route.name == 'admin_google_auth' ? 'col-12 ' : ''">Authentication Code</div>
        <div class="col-9">
          <q-input
            borderless
            type="text"
        ...
        <div class="col-3">
          ...
          <q-btn
            ...
            :class="$route.name == 'admin_google_auth' ? 'full-width' : ''"
            flat
    ...
    <div v-else class="text-center self-center" style="padding-top: 10vh">
      <img src="icon_layout/lock_01.png" style="width: 180px" />
        <div class="q-mb-sm text-h5" style="font-weight: 500">Google Authentication</div>
        <div class="q-my-xl" style="font-size: 18px">Used for withdrawals and security modification</div>
        <q-btn
          @click="is_disable_2fa = true"
          label="Disable"
          no-caps
          class="bg-secondary text-white text-bold q-py-xs"
...
import { postEnable2faAdmin, postDisable2faAdmin } from "../../../references/url";
...
export default {
  ...
  methods: {
    ...
    async checkUser2faStatus() {
      if (this.$admin_res.user_info.google_authenticator) {
        this.is_2fa_enabled = this.$admin_res.user_info.google_authenticator.is_2fa_enabled; // true
      } else {
    ...
    async enable2FA() {
      ...
      let res = await this.$_post(postEnable2faAdmin, query_inputs);
    ...
    async disable2FA() {
      let res = await this.$_post(postDisable2faAdmin, { otp: Number(this.otp) });
