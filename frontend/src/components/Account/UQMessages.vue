                ...
                <div v-else>
                    ...
                    <q-tab-panels v-model="tab" animated style="background: transparent !important;">
                        <q-tab-panel name="friends" style="background: transparent !important;">
                            <q-list>
                                <div v-for="(user, index) in ka_chat" :key="index" class="q-my-xs">
                                    <q-item
                                        v-if="tab == 'friends'"
                                        clickable
                                        v-ripple
                        ...
                        <q-tab-panel name="groups">
                            <q-list>
                                ...
                                :key="index">
                                    ...
                                    clickable
                                    v-ripple
                                    @click="chat_input_group(user.chat_id, user.user_name, index, user.chat_name, user.message_id)"
                                    :active="is_active && index == idx"
                                    active-class="my-menu-link"
                                    >
                            ...
                            <q-infinite-scroll reverse>
                                ...
                                <div v-touch-hold.mouse="hold_to_delete"  @mousedown="delete_message(message)"  v-if="message.sender == 'me'" style="float: right;">
                                    ...
                                    <div v-else-if="message.message_type == 'text'" class="message_sender">
                                        ...
                                        <q-btn
                                            class="gt-xs"
                                            v-if="hold_delete_btn == true &&  message_to_remove.message_id == message.message_id"
                                    ...
                                    <div v-else-if="message.message_type == 'custom_emoji'" @mousedown="delete_message(message)">
                    ...
                    <div id="preview" v-if="url_video" style="padding: 10px;">
                        <div class="docu_preview" style="position: relative;">
                            ...
                            <q-btn v-if="url_video != null"
                                ...
                                class="remove__btn"
                                size="10px"
                                style="position: absolute; right: 0; bottom: 35px;">
                            </q-btn>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    async mounted() {
        ...
        this.socket.on('new_message', (data) => {
            ...
            if(this.my_id.id == data.receiver_id){
                this.$q.notify({
                    message:`${data.receiver_username} messaged you.`,
                    color: 'purple',
        ...
        // this.socket.on('read_message', async (data) => {
        //     this.ka_chat = [];
    ...
    methods: {
        ...
        async chat_input(id, index, sender_id, sender_username,  receiver_id, receiver_username, last_active, message_id, clicked_message = '')
        {
            ...
            this.seller_name = sender_username;
        ...
        async getMessage()
        {
            ...
            res.data.message_details.forEach( item => {
                ...
                this.messages.push({
                    message_id: res.data.message_details[count].message_id,
            ...
            this.last_message = this.messages.slice(-1)[0].text;

            // search clicked to edit chi
            if (this.messages != [] && this.clicked_search != '' && document.getElementById('clicked_search') != null) {
                this.$nextTick(() => {
                    console.log(document.getElementById('clicked_search'), 'clicked_search');
        ...
        async sendEmoji(emoji)
        {
            ...
            if (res.status == 200) {
                this.socket.emit('new_message', {
                    message: this.message_details.newMessage,
                    sender: this.message_details.sender,
                    receiver: this.message_details.receiver,
                    receiver_id      :  this.message_details.receiver_id,
                    receiver_username: this.message_details.receiver_username
                });
        ...
        async sendMessage()
        {
            ...
            console.log('message details asd', this.message_details);
            ...
            if (res.status == 200) {
                this.socket.emit('new_message', {
                    ...
                    receiver_id      :  (this.my_id.id == this.message_details.receiver_id) ? this.message_details.sender_id : this.message_details.receiver_id,
                    receiver_username: this.message_details.sender_username
                });
                this.newMessage = '';
                this.message_details.newMessage = '';
                this.url_image = [];
        ...
        {
            let res = await this.$_post(readMessage, {
                chat_id     : chat_id,
